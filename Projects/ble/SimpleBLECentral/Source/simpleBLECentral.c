...
static void simpleBLEPeripheral_HandleSerial(mtOSALSerialData_t *cmdMsg)
{
  ...
  CMD = str[1];
  if ( CMD == '1' ) {
    // Start or stop discovery  开始或停止设备扫描
    if ( simpleBLEState != BLE_STATE_CONNECTED )
    {
      ...
      {
        GAPCentralRole_CancelDiscovery();   //取消扫描
      }
    }
  ...
  if ( CMD == '2' ) {
    // Display discovery results    显示发现结果
  ...
  if ( CMD == '5' ) {
    ...
    if ( simpleBLEState == BLE_STATE_CONNECTED )
    {
      ...
      {
        simpleBLERssi = FALSE;
        GAPCentralRole_CancelRssi( simpleBLEConnHandle ); //取消周期发送RSSI
