...
#ifndef NO_BLS_ESCAPE_DETECTION
#include "cb_esc.h"
#endif
...
/*  Directed advertisement is active for 1.2 seconds
    Wait a little bit longer than that to make sure everything is finished 
    before throwing away the data */
#define cbBLS_CONNECT_ON_DATA_TIMEOUT    (1500) //ms
...
typedef struct 
{
  ...
  uint8                 maxTxBytes;
  uint8                 bytesSentSinceConnect;
  bool                  disconnectOnTxComplete;
                     ...
                     uint16 bufSize)
{
  ...
  case cbBLS_S_IDLE:
      ...
      if (result == SUCCESS)
      {
          ...
          osal_CbTimerStart(connTimeout, NULL, cbBLS_CONNECT_ON_DATA_TIMEOUT, &(bls.connTimerId));
      }
    break;
...
uint8 cbBLS_setBytesToTransfer(uint8 nBytes)
{
   bls.maxTxBytes = nBytes;
   return SUCCESS;
}

uint8 cbBLS_setDisconnectOnTxComplete(bool on)
{
    bls.disconnectOnTxComplete = on; 
    return SUCCESS;
}
...
static void spsDataConfCallback(uint16 connHandle)
{
  ...
  case cbBLS_S_TX_IN_PROGRESS:
    ...
    bls.bytesSentSinceConnect += bls.writeBufCurrentSize;
    
    if(bls.disconnectOnTxComplete)
    {
        if(bls.maxTxBytes != 0 && bls.bytesSentSinceConnect >= bls.maxTxBytes)
        { 
...
static void connTimeout(uint8* idp)
{
    uint8 nBytes = bls.writeBufTotalSize;
    ...
    /* Call WriteComplete in order to throw away the pending buffer and proceed. */
    blsCallbackNotifyWriteComplete(cbBLS_PORT_0, nBytes);
}
